# ============================================================
# EfficientDet-Lite0 + Windows + Webcam (OpenCV)
...
SCORE_THRESHOLD = 0.5          # 탐지 신뢰도 기준 (0.0 ~ 1.0)
                               #   낮추면 → 더 많이 탐지 (오탐 증가)
                               #   높이면 → 더 엄격하게 탐지 (미탐 증가)
CAMERA_INDEX    = 0            # 웹캠 장치 번호 (0: 기본 웹캠)
VIDEO_PATH      = './sample.mp4'  # 동영상 파일 경로 (None 이면 웹캠 사용)
...
def load_model(model_path: str):
    ...
    for i, d in enumerate(output_details):
        print(f"[INFO] output[{i}]: name={d['name']}, shape={d['shape']}")
...
# 4. 카메라/동영상 초기화 함수
# ──────────────────────────────────────────
def init_camera(camera_index: int, img_size: int,
                video_path: str = None) -> cv2.VideoCapture:
    """
    OpenCV VideoCapture로 웹캠 또는 동영상 파일을 초기화합니다.

    Args:
        camera_index: 웹캠 장치 번호 (video_path 미사용 시)
        img_size    : 캡처 해상도 (웹캠 전용)
        video_path  : 동영상 파일 경로 (None 이면 웹캠 사용)
    Returns:
        cap: 초기화된 VideoCapture 객체
    """
    if video_path:
        ...
        print(f"[INFO] Camera started successfully (index={camera_index})")
...
def preprocess(frame: np.ndarray, input_dtype) -> np.ndarray:
    ...
    Args:
        frame      : 카메라에서 받은 원본 이미지 (H, W, 3)  BGR
        input_dtype: 모델이 기대하는 입력 데이터 타입
    Returns:
        img: 모델 입력용 배열 (1, H, W, 3)  RGB
    """
    # BGR → RGB 변환 (OpenCV는 BGR, 모델은 RGB)
...
def main():
    ...
    cap         = init_camera(CAMERA_INDEX, IMG_SIZE, VIDEO_PATH)
    ...
    try:
        while True:
            ...
            # Step 2: 전처리 (BGR→RGB 변환 + 리사이즈 + 타입 변환)
            ...
            # Step 5: 표시용 이미지 준비 (원본 frame을 320x320으로 리사이즈)
            display_img = cv2.resize(frame, (IMG_SIZE, IMG_SIZE))

            # Step 6: 탐지 결과 이미지에 그리기
